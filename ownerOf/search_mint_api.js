...
async function fetchTokenOwnersWithMintTime() {
    const response = await axios.get('https://explorer.bsquared.network/api/?module=logs&action=getLogs&address=0x3FdaCd1C4fCbF43568C5f3d9E674aE9C9ba30847&fromBlock=2239514&toBlock=2256564&page=1&offset=1000&topic0=0xc6968bf6c0171f55b87087ceecec6075320b774e7f6cca631eb41fcf8da17b68&api_key=Bearer 291afe4e-a798-4075-8422-60755a8cfdf3');
    ...
    if (response.data.status === "1") {
        ...
        for (const transaction of transactions) {
            ...
            const nftMintedEvent = nftContract.interface.getEvent("NFTMinted");
            ...
            for (const log of eventLog.logs) {
                if (log.topics.includes(topic)) {
                    const address = ethers.BigNumber.from(log.topics[1]).toHexString();
