...
contract linkstarterPublicSale is Ownable, ReentrancyGuard {
    ...
    event ClaimBTC(address indexed user, uint256 amount);
    event WhiteAccountAdded(address indexed account);
    event WhiteAccountRemoved(address indexed account);

    event ParametersSet(uint256 joinIdoPrice, uint256 rewardBTCAmount);
    event StartSet(bool bstart, uint256 startTime);
    event TimeSet(uint256 idoTimeRange, uint256 claimDt, uint256 period);
    event WhiteAddrSet(bool bWhiteAddr);
    ...
    returns (address WhiteAddress)
    {
        require(
            ...
            "LinkStarterPublicSale: exceeded the length index of the whitelist collection"
        );
    ...
    function claimToken() external nonReentrant {
        ...
        if (expectedAmount > 0) {
            ...
            emit ClaimTokens(msg.sender, expectedAmount);
        }
    ...
    function claimBTC() external nonReentrant {
        ...
        if (refundAmount > 0) {
            ...
            emit ClaimBTC(msg.sender, refundAmount);
        }
    }
    ...
    function setParameters(
        ...
        require(rewardTokenAddr != address(0), "LinkStarterPublicSale: rewardTokenAddr is zero address");
        require(joinIdoPrice0 > 0, "LinkStarterPublicSale: joinIdoPrice0 is zero");
        require(rewardBTCAmount0 > 0, "LinkStarterPublicSale: rewardBTCAmount0 is zero");

        rewardToken = IERC20(rewardTokenAddr);
        joinIdoPrice = joinIdoPrice0;
        rewardBTCAmount = rewardBTCAmount0;

        emit ParametersSet(joinIdoPrice0, rewardBTCAmount0);
    }
    ...
    function setStart(bool bstart, uint256 startTime0) external onlyOwner {
        ...
        emit StartSet(bstart, startTime0);
    }
    ...
    function setDt(
        ...
        require(_claimDt > _idoTimeRange, "LinkStarterPublicSale: claimDt must be greater than idoTimeRange");
        idoTimeRange = _idoTimeRange;
        claimDt = _claimDt;
        claimPeriod = period;

        emit TimeSet(_idoTimeRange, _claimDt, period);
    }
    ...
    function addWhiteAccount(address account) external onlyOwner {
        ...
        emit WhiteAccountAdded(account);
    }
    ...
    function addWhiteAccount(address[] calldata accountArr) external onlyOwner {
        for (uint256 i = 0; i < accountArr.length; ++i) {
            ...
            emit WhiteAccountAdded(accountArr[i]);
        }
    }
    ...
    function removeWhiteAccount(address account) external onlyOwner {
        ...
        for (uint256 i = 0; i < _WhiteAddrArr.length; ++i) {
            if (_WhiteAddrArr[i] == account) {
                ...
                emit WhiteAccountRemoved(account);
                break;
            }
        }
    }
