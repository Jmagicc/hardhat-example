...
contract Pizzapad is Ownable, ReentrancyGuard {
    ...
    uint256 public lastClaimTime; //newAdd: Last claim time
    ...
    constructor(
        ...
        claimDt1 = dt + 10 * 60; // 10 minutes delay for first claim
...
// 修改 getParameters 函数，实现每隔5分钟线性释放的逻辑
    function getParameters(address account) public view returns (uint256[] memory) {
        ...
        if (block.timestamp > linearReleaseStartTime) {
            if (_alreadyClaimNumArr[account] == 0) {
                ...
                uint256 intervals = (block.timestamp - lastClaimTime) / 300; // Calculate intervals (5 minutes = 300 seconds)
                uint256 totalReleasablePercentage = 20 * intervals;
    ...
    function claimToken() external nonReentrant {
        require(mbStart, "Pizzapad: not Start!");
        require(block.timestamp > startTime + dt, "Pizzapad: need end!");
        require(_balance[msg.sender] > 0, "Pizzapad: balance zero");
        require(block.timestamp > startTime + claimDt1, "Pizzapad: need begin claim!");
        ...
        if (_alreadyClaimNumArr[msg.sender] == 0 && block.timestamp > linearReleaseStartTime) {
            ...
            lastClaimTime = block.timestamp; // 记录第一次领取时间
        } else {
            // 线性释放逻辑
            uint256 elapsedTime = block.timestamp - lastClaimTime;
            uint256 intervals = elapsedTime / 300; // 5 minutes interval (300 seconds)

            // 计算应该释放的总量
            uint256 totalReleasable = totalExpectedAmount * intervals * 20 / 100; // 每个间隔释放20%
            // 减去已经领取的数量
            amountToClaim = totalReleasable - (totalExpectedAmount - _balance[msg.sender]);

            // 更新已领取次数
            _alreadyClaimNumArr[msg.sender] = intervals + 1;
            lastClaimTime = lastClaimTime + intervals * 300; // 更新上次领取时间
        }
    ...
    function setStart(bool bstart) external onlyOwner {
        ...
        linearReleaseStartTime = startTime + dt + 10 * 60; // 10 minutes delay for first claim
